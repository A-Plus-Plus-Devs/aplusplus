...
{
    printf("[DEBUG] Starting to parse tokens\n");
    
    if (!parser || !parser->current_token)
    {
        printf("[DEBUG] Error: Parser or current token is NULL\n");
        return NULL;
    }
    ...
    printf("[DEBUG] Finished parsing tokens\n");
    return root;
}
...
static ASTNode *parse_expression(Parser *parser)
{
    ...
    // Keep parsing binary operators as long as we find them
           ...
           parser->current_token->type == TOKEN_POWER)
    {
        ...
        printf("[DEBUG] Found binary operator: %d\n", op_type);
        
        get_next_token(parser); // consume operator
        
        ASTNode *right;
        if (op_type == TOKEN_POWER) {
        ...
        if (!right)
        {
            printf("[DEBUG] Failed to parse right term of expression\n");
            free_ast(left);
            return NULL;
        }
        ...
        switch (op_type)
        {
            ...
            case TOKEN_POWER:
                ...
                break;
            default:
                printf("[DEBUG] Error: Unknown operator type: %d\n", op_type);
                free_ast(left);
                free_ast(right);
                return NULL;
        }

        ASTNode *binary_op = create_node(NODE_BINARY_OP, left, right, op);
        if (!binary_op)
        {
            printf("[DEBUG] Error: Failed to create binary operation node\n");
            free_ast(left);
            free_ast(right);
            return NULL;
        }
...
static ASTNode *parse_yield_statement(Parser *parser)
{
    printf("[DEBUG] Parsing yield statement\n");
    ...
    if (!expr)
    {
        printf("[DEBUG] Error: Failed to parse yield expression\n");
        return NULL;
    }

    if (parser->current_token->type != TOKEN_SEMICOLON)
    {
        printf("[DEBUG] Error: Expected semicolon after yield expression\n");
        free_ast(expr);
        return NULL;
    }
    get_next_token(parser); // consume ;
